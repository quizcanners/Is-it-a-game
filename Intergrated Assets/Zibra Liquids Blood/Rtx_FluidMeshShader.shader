Shader "ZibraLiquids/RTX FluidMeshShader"
{
    SubShader
    {
        Pass
        {
          ...
          //  sampler2D _CameraDepthTexture;
...
#if defined(CUSTOM_REFLECTION_PROBE)
            ...
            #include <Assets\Plugins\Zibra\Liquids\Shaders\RenderingUtils.cginc>
            ...
            PSOut PSMain(VSOut input)
            {
              ...
              //  float fresnel = FresnelTerm(materialMetalness, NV);
               ...
               // float3 lightColor = _LightColor0;
                ...
                float3 col = 0;
              /*  col += ReflectionColor.xyz * fresnel * ReflectedColor / Average(ReflectionColor.xyz);
                col += fresnel*ComputeMaterial(cameraPos, cameraRay, normal, lightDirWorld, lightColor, materialRoughness);
                col += materialEmission;
                col += (1.0 - fresnel) * RefractionRay(worldPos, cameraRay, normal, input.uv, false);*/
                
                float shadow = 1;

                float fresnel = smoothstep(0.5, 1, dot(-cameraRay, normal));
                ...
                float3 bakeReflected = SampleReflection(worldPos, -cameraRay, normal, shadow, reflectionPos, outOfBoundsRefl);
                TopDownSample(reflectionPos, bakeReflected, outOfBoundsRefl);

                float outOfBoundsStraight;
                float3 straightHit;
                float3 bakeStraight = SampleRay(worldPos, normalize(cameraRay - normal * 0.2), shadow, straightHit, outOfBoundsStraight);
                ...
                col = _qc_BloodColor.rgb * col * 0.5
                    + lerp(_qc_BloodColor.rgb * bakeReflected, (
                        //farFromSurface + 
                        _qc_BloodColor.rgb
                        ) * 0.5 * bakeStraight, showStright);
              ...
              //  col.rgb = _qc_BloodColor.rgb * col.rgb
                ...
                ApplyBottomFog(col.rgb, worldPos, input.raydir);


                output.color = float4(clamp(col, 0., 10000.0), 1.0);
                return output;
            }
            ENDHLSL
        }
